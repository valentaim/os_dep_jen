...
echo -en \"VLAN=yes\nDEVICE=eth0.100\nTYPE=Ethernet\nONBOOT=yes\nIPADDR=10.0.100.${last_octet} NETMASK=255.255.255.0\" > /etc/sysconfig/network-scripts/ifcfg-eth0.100;
echo -en \"VLAN=yes\nDEVICE=eth0.101\nTYPE=Ethernet\nONBOOT=yes\nIPADDR=10.0.101.${last_octet} NETMASK=255.255.255.0\" > /etc/sysconfig/network-scripts/ifcfg-eth0.101;
echo -en \"VLAN=yes\nDEVICE=eth0.102\nTYPE=Ethernet\nONBOOT=yes\nIPADDR=10.0.102.${last_octet} NETMASK=255.255.255.0\" > /etc/sysconfig/network-scripts/ifcfg-eth0.102;
echo -en \"VLAN=yes\nDEVICE=eth0.103\nTYPE=Ethernet\nONBOOT=yes\nIPADDR=10.0.103.${last_octet} NETMASK=255.255.255.0\" > /etc/sysconfig/network-scripts/ifcfg-eth0.103;
echo -en \"VLAN=yes\nDEVICE=eth0.104\nTYPE=Ethernet\nONBOOT=yes\nIPADDR=10.0.104.${last_octet} NETMASK=255.255.255.0\" > /etc/sysconfig/network-scripts/ifcfg-eth0.104;
...
knife ssh 'role:ha-controller1' chef-client -x root
echo "Deploying second controller"
knife ssh 'role:ha-controller2' chef-client -x root
echo "ReDeploying first controller"
knife ssh 'role:ha-controller1' chef-client -x root
echo "Deploying compute nodes"
knife ssh 'role:single-compute' chef-client -x root
